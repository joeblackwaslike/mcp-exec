...
After adding, those vars are forwarded into all sandbox exec calls automatically. You can also pass them on a per-call basis via the `runtime.env` config object (see [Per-call environment variables](/guide/configuration#per-call-environment-variables)).
...
- [Configuration →](/guide/configuration) — full `sandbox` block reference
- [Security →](/guide/security) — why env filtering matters
