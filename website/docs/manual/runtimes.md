...
**MCP imports:** `import { tool } from 'mcp/server-name'` — async/await, same as any ES module.
...
**Return value:** stdout. Use `echo` or command output directly; everything written to stdout is captured as `result`.
...
**MCP access:** Available via HTTP bridge on `127.0.0.1`. Import tools with `from mcp.<server_name> import <tool_name>` — the sandbox auto-generates a local `mcp/` package and injects it into `PYTHONPATH`. All MCP calls are synchronous (no `await`).
...
When running from Codex CLI, all three runtimes work identically from mcp-exec's perspective.

Codex CLI applies a platform-native sandbox (`sandbox-exec` on macOS, `bwrap` on Linux) that wraps the mcp-exec process itself. mcp-exec's `srt` sandbox wraps code inside `exec()`. These are **independent layers** — you get double sandboxing, which is intentional and good.

No special configuration is needed for runtime behavior. Sandbox policy config is the same regardless of whether you're running from Claude Code or Codex CLI — set in `settings.json` as described in [Configuration → Codex CLI](/docs/guide/configuration#configuration-for-codex-cli).

## Related

- [exec()](/docs/manual/exec) — full parameter reference
- [Sessions](/docs/manual/sessions) — how Node session state works across calls
- [Examples](/docs/manual/examples) — complete workflows per runtime
