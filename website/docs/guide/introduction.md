...
mcp-exec adds two tools to your agent:
...
**`exec(code, runtime)`** — runs code in an OS-level sandbox. MCP servers are importable as modules inside the sandbox. Multiple tool calls, data processing, filtering, and aggregation happen entirely inside the sandbox — only the final return value comes back to the agent.

The sandbox is opaque by design. The agent sees the input (the code it sent) and the output (the final return value). Everything in between — the raw API responses, the intermediate arrays, the filtered lists, the fetched document bodies — stays inside the sandbox and is discarded when the call completes.
...
:::info Real benchmark
On a GitHub + Slack standup workflow: **43,800 → 90 tokens (99.8% reduction)**. The 847 issue objects, 12 PR objects, and 48 Slack messages were computed and discarded entirely inside the sandbox — never touching context.
:::
...
|------|------|-------------|
| 1 | `searchInvoices()` → 847 invoice objects | ~14,200 |
| 2 | Claude filters in-context → filtered list displayed | ~2,000 |
| 3 | `getCustomer()` × 23 calls → 23 customer records | ~8,100 |
| 4 | `createDraft()` → draft confirmation | ~400 |
...
| Step | Call | Tokens added |
|------|------|-------------|
| 1 | `exec({ runtime: "node", code: "..." })` → all four steps inside the sandbox | ~80 |
...
:::tip
The 99% reduction isn't a compression trick. The data was simply never there.
:::
...
- **Single-tool calls where the result is small and you want it visible in context** — if you're fetching one record and you want the agent to reason about its contents directly, keep it in context.
...
:::warning
`exec()` is not a general-purpose code runner. It's designed for MCP orchestration workflows. Running arbitrary untrusted code — even inside the sandbox — carries risk. See [Security](/docs/guide/security) for what the sandbox does and does not protect against.
:::

## Next steps

- [Install mcp-exec →](/docs/guide/installation)
